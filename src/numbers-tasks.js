...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosVectors = numerator / denominator;
  const angle = Math.acos(cosVectors);
  return angle;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 0; i < index; i += 1) {
    const tempNumber = nextFibonacciNumber;
    nextFibonacciNumber = previousFibonacciNumber + nextFibonacciNumber;
    previousFibonacciNumber = tempNumber;
  }
...
function isNumber(number) {
  if (Number.isFinite(number) && typeof number === 'number') {
    return true;
  }
  return false;
}
